...
int main( void )
{
    ...
    // The name of a 1-d array itself is of type
    // "const pointer to element type",
    // so the initialization of the array's three int * elements works
    int *tricks[3] = { m2, m3, m1 }; // shuffled so not consecutive in RAM
    ...
    cout << "tricks is:\n" << flush;
    for( i = 0; i < 3; i++ )     // for each "row" (pointer to a row)
    {
    ...
    // why can't I just put mPtr = matrix below instead of using & and [0][0]?
    mPtr = &matrix[0][0];       // get base of matrix
    for( i = 0; i < 3; i++ )    // for each row
    {
        for( j = 0; j < 4; j++ ) // for each column
    ...
    //
    // NOTE! ---------------------------------------------------------
    // DO NOT DO THIS! EVER! ALWAYS treat a 2-d array as a 2-d array!
    // ---------------------------------------------------------------
    //
